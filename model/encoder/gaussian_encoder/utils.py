...
def cartesian(anchor, pc_range, use_sigmoid=True, contracted=-1.0):
    if use_sigmoid:
        xyz = safe_sigmoid(anchor[..., :3])
    else:
        xyz = anchor[..., :3].clamp(min=1e-6, max=1-1e-6)
    if contracted > 0.0:
        xyz = contract_s2x(xyz * 2 - 1, pc_range, contracted)
    else:
...
def reverse_cartesian(xyz, pc_range, use_sigmoid=True, contracted=-1.0):
    if contracted > 0.0:
        unitxyz = contract_x2s(xyz, pc_range, contracted) / 2 + 0.5
    else:   
