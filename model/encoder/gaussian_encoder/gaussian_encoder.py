...
class GaussianOccEncoder(BaseEncoder):
    def __init__(
        ...
        **kwargs,
        ...
        deformable_model["contracted"] = contracted
        refine_layer["contracted"] = contracted
        spconv_layer["contracted"] = contracted
        self.op_config_map = {
    ...
    def forward(
        ...
        **kwargs
        ...
        if anchor.shape[-1] == 3:
            assert self.operation_order[-1] == 'refine'
            with torch.no_grad():
                anchor_attr = self.layers[-1](instance_feature, anchor, anchor_embed)[-1]
                anchor = torch.cat([
                    anchor,
                    anchor_attr.scales,
                    anchor_attr.rotations,
                    anchor_attr.colors,
                    anchor_attr.opacities,
                ], dim=-1)
