...
class GaussianLifterV3(BaseLifter):
    def __init__(
        self,
        num_anchor,
        embed_dims,
        ...
        **kwargs,
        ...
        self.register_buffer("depth_bins", torch.linspace(
            1.0, 80.0, self.num_samples + 1, dtype=torch.float), persistent=False)
    ...
    def forward(self, metas, **kwargs):
        ...
        #     ds = metas['image_wh'][:, :, None, None]
        #     ds[..., 0] = ds[..., 0] / w
        #     ds[..., 1] = ds[..., 1] / h
        #     jitter = torch.rand_like(uv) * (ds - 1.0) - ds / 2.0
        #     uv += jitter
        ...
        # ) # b, n, h, w, a, 3
        # pdfs[..., -2] += pdfs[..., -1]
        sampled_anchor = (anchor_pts * pdfs[..., None]).sum(dim=-2, keepdim=True)
