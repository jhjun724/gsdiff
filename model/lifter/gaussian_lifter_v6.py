...
from ..utils.contract import contract_x2s, contract_s2x
...
class GaussianLifterV6(BaseLifter):
    def __init__(
        self,
        num_anchor,
        embed_dims,
        ...
        **kwargs,
        ...
        self.register_buffer("depth_bins", torch.linspace(
            1.0, 72.0, 2, dtype=torch.float), persistent=False)
    ...
    def get_uniform_points_along_rays(self, points, n):
        near = points[..., 0, :]
        far = points[..., 1, :]

        steps = torch.linspace(0, 1, n, device=points.device, dtype=points.dtype)

        steps = steps.view(1, 1, 1, 1, n, 1)
        near = near.unsqueeze(-2)
        far = far.unsqueeze(-2)

        sampled_points = near + steps * (far - near)
    ...
    def forward(self, metas, **kwargs):
        ...
        uvd = uv.unsqueeze(4).expand(b, n, h, w, 2, 2)
        ...
        if self.contracted > 0.0:
            anchor_pts = contract_x2s(anchor_pts, self.pc_range, self.contracted) / 2 + 0.5
            sampled_anchor = self.get_uniform_points_along_rays(anchor_pts, pdfs.shape[-1])
            sampled_anchor = contract_s2x(sampled_anchor * 2 - 1, self.pc_range, self.contracted)
            sampled_anchor = (sampled_anchor * pdfs[..., None]).sum(dim=-2, keepdim=True)
        else:
            sampled_anchor = self.get_uniform_points_along_rays(anchor_pts, pdfs.shape[-1])
            sampled_anchor = (sampled_anchor * pdfs[..., None]).sum(dim=-2, keepdim=True)
        disable_mask = pdfs[..., -1] > 0.5
