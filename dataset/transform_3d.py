...
class ResizeCropFlipImage(object):
    def __call__(self, results):
        ...
        for i in range(N):
            ...
            img2lidar = np.linalg.inv(results["lidar2img"][i])
            if "cam_positions" in results:
                results["cam_positions"][i] = (img2lidar @ results["cam_intrinsic"][i] @
                    np.array([0., 0., 0., 1.]).reshape([4, 1])).flatten()[:3]
            if "focal_positions" in results:
                results["focal_positions"][i] = (img2lidar @ results["cam_intrinsic"][i] @
                    np.array([0., 0., f, 1.]).reshape([4, 1])).flatten()[:3]

        results["img"] = new_imgs
        results["img_shape"] = [x.shape[:2] for x in new_imgs]
        results["pad_shape"] = [x.shape for x in new_imgs]
        results["pad_before_shape"] = [x.shape for x in new_imgs]
        return results
...
class LoadMultiViewImageFromFiles(object):
    ...
    def __call__(self, results):
        ...
        results['pad_before_shape'] = img.shape
