...
def save_occ(
    ...
    filepath = os.path.join(save_dir, 'occ', f'{name}.png')
    os.makedirs(os.path.join(save_dir, 'occ'), exist_ok=True)
    if offscreen:
...
def save_gaussian(save_dir, gaussian, name, scalar=1.5, ignore_opa=False, filter_zsize=False):
    ...
    filepath = os.path.join(save_dir, 'gaussian', f'{name}.png')
    os.makedirs(os.path.join(save_dir, 'gaussian'), exist_ok=True)
...
def save_gaussian_topdown(save_dir, anchor_init, gaussian, name):
    ...
    os.makedirs(os.path.join(save_dir, 'gaussian_topdown'), exist_ok=True)
    plt.savefig(os.path.join(save_dir, 'gaussian_topdown', f"{name}.jpg"))
    plt.clf(); plt.cla()

def get_denormalized_img(img, mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375]):
    assert isinstance(mean, list) and isinstance(std, list)
    assert len(img.shape) == 5 and img.shape[2] == 3
    if isinstance(img, torch.Tensor):
        mean = torch.tensor(mean).to(img.device).view(1, 1, -1, 1, 1)
        std = torch.tensor(std).to(img.device).view(1, 1, -1, 1, 1)
        img = img * std + mean
    else:
...
def save_rendered_img(
    ...
    if rgbs is not None:
        assert (len(rgbs.shape) == 6) and (rgbs.shape[3] == 3), \
            "rgbs should be (batch_size, num_gaussians, num_cams, 3, height, width)"
    if depths is not None:
        assert (len(depths.shape) == 5), \
            "depths should be (batch_size, num_gaussians, num_cams, 1, height, width)"
    if features is not None:
        assert (len(features.shape) == 6), \
            "features should be (batch_size, num_gaussians, num_cams, C, height, width)"
    if gt is not None:
        assert (len(gt.shape) == 5), \
            "gt should be (batch_size, num_cams, 3, height, width)"
    assert rgbs is not None or depths is not None, \
        "At least one of rgbs and depths should be provided"
    if rgbs is not None:
        os.makedirs(f'{save_dir}/rgb', exist_ok=True)
        rgbs = rgbs.permute(0, 1, 2, 4, 5, 3).detach().clone().cpu().numpy()
        batch_size, num_gs, num_cams = rgbs.shape[:3]
    if depths is not None:
        os.makedirs(f'{save_dir}/depth', exist_ok=True)
        depths = depths.to(torch.float32).detach().clone().cpu().numpy()
        depths = (depths - depths.min()) / (depths.max() - depths.min())
        batch_size, num_gs, num_cams = depths.shape[:3]
    if features is not None:
        os.makedirs(f'{save_dir}/feature', exist_ok=True)
        batch_size, num_gs, num_cams, C, H, W = features.shape
        features = features.permute(0, 1, 2, 4, 5, 3).flatten(0, 4)
        features = get_robust_pca(features)
        features = features.view(batch_size, num_gs, num_cams, H, W, 3)
        features = features.detach().clone().cpu().numpy()
    if gt is not None:
        os.makedirs(f'{save_dir}/gt_rgb', exist_ok=True)
        gt = gt.permute(0, 1, 3, 4, 2).detach().clone().cpu().numpy()
        mean = np.array([123.675, 116.28, 103.53]).reshape(1, 1, 1, 1, 3)
        std = np.array([58.395, 57.12, 57.375]).reshape(1, 1, 1, 1, 3)
        gt = (gt * std + mean) / 255.

    for bi in range(batch_size):
        for gi in range(num_gs):
            if rgbs is not None:
                ...
                imageio.imwrite(
                    f'{save_dir}/rgb/{index[bi]:04d}_{token[bi]}_rgb_{gi}.png',
            ...
            if depths is not None:
                ...
                imageio.imwrite(
                    f'{save_dir}/depth/{index[bi]:04d}_{token[bi]}_depth_{gi}.png',
                    (merged_depth * 255).astype('uint8')
                )
            if features is not None:
                merged_feature = np.concatenate([
                    np.concatenate([
                        features[bi, gi, 2],
                        features[bi, gi, 0],
                        features[bi, gi, 1]
                ...
                imageio.imwrite(
                    f'{save_dir}/feature/{index[bi]:04d}_{token[bi]}_feature_{gi}.png',
                    (merged_feature * 255).astype('uint8')
                )
        if gt is not None:
            merged_gt = np.concatenate([
                np.concatenate([
                    gt[bi, 2],
                    gt[bi, 0],
                    gt[bi, 1]
            ...
            imageio.imwrite(
                f'{save_dir}/gt_rgb/{index[bi]:04d}_{token[bi]}_gt_rgb.png',
                (merged_gt * 255).astype('uint8')
            )
